...
export async function fetchOrderList(): Promise<{ header: OrderHeader }[]> {
  try {
    const order = await apiFetch<OrderDetail>('/api/orders/1')
    return [
      { header: order.header },
      { header: { ...order.header, number: 4510001115 } },
      { header: { ...order.header, number: 4510001116 } },
      { header: { ...order.header, number: 4510001117 } },
    ]
  } catch {
    const mock = {
      header: {
    ...
    return [mock, { ...mock, header: { ...mock.header, number: 4510001115 } }, { ...mock, header: { ...mock.header, number: 4510001116 } }, { ...mock, header: { ...mock.header, number: 4510001117 } }]
  }
}

export async function fetchOrderDetail(): Promise<OrderDetail> {
  try {
    return await apiFetch<OrderDetail>('/api/orders/1')
  } catch {
    return {
      header: {
        number: 4510001114,
